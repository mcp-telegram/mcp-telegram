...
- Custom error types (`NetworkError`, `AuthError`, `RateLimitError`) for better error categorization
- Automatic retry logic with exponential backoff for transient network errors
- Enhanced error handling in critical API methods (`connect`, `sendMessage`, `downloadMedia`, `getDialogs`, etc.)
- Unit tests for error detection and retry logic
- Documentation for network error handling improvements (`NETWORK_ERROR_HANDLING.md`)

### Changed
- API calls now automatically retry up to 3 times on network timeouts before failing
- Error messages now differentiate between network issues, auth failures, and rate limits
- Connection errors provide more specific guidance based on error type
