...
Скачать с [GitHub Releases](https://github.com/mcp-telegram/mcp-telegram/releases/latest):
...
## Транспорт WSS (порт 443)

Некоторые диапазоны IP у VPS/хостингов (например, у части провайдеров Hostinger, OVH и подобных shared-VPS) блокируют исходящий порт `80` к IP-блокам дата-центров Telegram в рамках анти-абуз политики, при этом те же дата-центры остаются доступны по порту `443`. Стандартный MTProto-транспорт TCPFull подключается по порту `80`, поэтому на таких хостах клиент навсегда зависает в `Connecting to ...:80/TCPFull...`.

Установите `TELEGRAM_USE_WSS=true`, чтобы переключить транспорт на порт `443`:
...
| Переменная | Описание |
|------------|----------|
| `TELEGRAM_USE_WSS` | Если `true`, gramJS использует порт `443` вместо `80` для MTProto-транспорта TCPFull. По умолчанию: `false`. Нельзя сочетать с `TELEGRAM_PROXY_*` (ограничение gramJS) — если заданы оба, `useWSS` игнорируется, приоритет у прокси (выводится предупреждение). |
