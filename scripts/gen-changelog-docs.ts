 ...
 *
 * These three pages are gitignored derived artifacts (like dist/). They are
 * regenerated by predocs:build on every dev/CI/deploy run, so they always
 * track CHANGELOG.md and can never drift — which matters because release-please
 * updates CHANGELOG.md on each release without knowing about this generator.
 *
 * Run with no args to write the files; pass --check to exit 1 when the output
 * differs from what's on disk (handy locally). Invoked from predocs:build.
 *
