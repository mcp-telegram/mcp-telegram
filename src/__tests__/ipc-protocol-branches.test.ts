...
describe("parseMessages — edge cases", () => {
  it("malformed JSON line is skipped, valid lines still parsed", () => {
    const good = encodeMessage({ id: "1", tool: "ping", args: {} });
    const buf = `not-valid-json\n${good}`;
    const { messages, remaining } = parseMessages(buf);
    assert.strictEqual(messages.length, 1);
    assert.strictEqual((messages[0] as { id: string }).id, "1");
    assert.strictEqual(remaining, "");
  });

  it("blank lines between messages are skipped", () => {
    const m1 = encodeMessage({ id: "a", tool: "foo", args: {} });
    const m2 = encodeMessage({ id: "b", tool: "bar", args: {} });
    const buf = `${m1}\n\n${m2}`;
  ...
  it("multiple complete messages in one chunk all parsed", () => {
    const msgs = [
      encodeMessage({ id: "1", tool: "a", args: {} }),
      encodeMessage({ id: "2", result: "ok" }),
      encodeMessage({ id: "3", error: "boom" }),
    ].join("");
...
describe("encodeMessage", () => {
  it("produces newline-terminated JSON", () => {
    const encoded = encodeMessage({ id: "1", tool: "test", args: { x: 1 } });
  ...
  it("round-trips through parseMessages", () => {
    const original = { id: "abc", result: { content: [{ type: "text", text: "hello" }] } };
