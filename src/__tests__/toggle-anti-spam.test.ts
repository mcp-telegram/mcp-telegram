...
describe("TelegramService.toggleAntiSpam", () => {
  it("invokes channels.ToggleAntiSpam with enabled=true for supergroup", async () => {
    ...
    const call = invocations.find((r) => r instanceof Api.channels.ToggleAntiSpam) as
      | Api.channels.ToggleAntiSpam
      | undefined;
  ...
  it("rejects broadcast channels (not megagroup)", async () => {
    ...
    assert.strictEqual(
      invocations.find((r) => r instanceof Api.channels.ToggleAntiSpam),
      undefined,
